...
//Twitch Madness
const alexWhiteRa = new wrestlerData(
 'Twitch Madness',
 'Alex WhiteRa',
 '-',
...
const arina_gp = new wrestlerData('Twitch Madness', 'arina_gp', '-', 0,4,0,0,1,0,2,2,1);
const balabama = new wrestlerData('Twitch Madness', 'Balabama', '-', 3,3,0,0,0,0,1,1,1);
const bigF = new wrestlerData('Twitch Madness', 'Big F', '-', 5,2,0,0,0,1,0,7,4);
const boogeyman = new wrestlerData('Twitch Madness', 'Boogeyman', '-', 0,2,0,0,3,4,0,1,0);
const bratishkin = new wrestlerData('Twitch Madness', 'Bratishkin', '89 Freaks', 3,4,2,4,1,0,0,1,1);
...
const cake = new wrestlerData('Twitch Madness', 'Cake', 'ARMA', 1,3,1,2,0,0,0,5,1);
const wlg = new wrestlerData('Twitch Madness', 'Denis WELOVEGAMES', 'Success', 3,1,3,1,0,0,5,5,1);
const dread = new wrestlerData('Twitch Madness', 'Dread', 'Танкисты', 1,0,7,1,0,1,1,1,0);
const faridysha = new wrestlerData('Twitch Madness', 'Faridysha', '-', 4,0,0,0,0,1,1,3,2);
const flashko = new wrestlerData('Twitch Madness', 'F1ashko', 'Uzyashko', 1,0,6,3,0 ,0,0,1,2);
const forsen = new wrestlerData('Twitch Madness', 'Forsen', '-', 3,3,0,0,1,2,0,3,1);
const golovach = new wrestlerData('Twitch Madness', 'Golovach', 'Cardmen', 0,1,0,1,0,0,0,1,0);
const guitman = new wrestlerData('Twitch Madness', 'Guit88man', 'Танкисты', 2,0,7,1,0,3,0,2,1);
const krabick = new wrestlerData('Twitch Madness', 'Krabick', '-', 4,0,0,0,4,4,1,1,0);
...
const lizon = new wrestlerData('Twitch Madness', 'Liz0n', '-', 3,4,0,0,1,0,2,1,0);
const maxgoodoq = new wrestlerData('Twitch Madness', 'Max Goodoq', 'ARMA', 4,3,1,2,0,0,0,1,0);
const melharucos = new wrestlerData('Twitch Madness', 'Melharucos', '-', 2,3,0,0,1,1,1,3,2);
const ilame = new wrestlerData('Twitch Madness', 'Mister iLame', 'Танкисты', 4,2,0,0,0,1,1,4,2);
const nuke = new wrestlerData('Twitch Madness', 'Nuke', '-', 0,4,0,0,0,2,0,2,0);
const ramona = new wrestlerData('Twitch Madness', 'Ramona', '-', 1,3,0,0,1,0,1,4,3);
...
const sodapoppin = new wrestlerData('Twitch Madness', 'Sodapoppin', '-', 1,1,0,0,0,1,0,0,0);
const speedyKarmik = new wrestlerData('Twitch Madness', 'Speedy Karmik', '-', 6,3,0,1,0,0,1,4,2);
const uncleBjorn = new wrestlerData('Twitch Madness', 'Uncle Bjorn', '-', 1,7,0,0,0,0,2,4,1);
const usachman = new wrestlerData('Twitch Madness', 'Usachman', 'WhoPG', 1,1,3,4,1,0,0,0,1);
...
const alinarin = new wrestlerData('Vision', 'AlinaRin', '-', 4,1,0,0,0,1,0,4,2);
const logvinov = new wrestlerData('Vision', 'Anton Logvinov', 'Gamer Industry', 4,3,2,3,0,0,0,1,1);
const arrowwoods = new wrestlerData('Vision', 'Arrowwoods', '-', 6,4,0,0,1,0,1,4,3);
const drDisrespect = new wrestlerData('Vision', 'Dr. Disrespect', '-', 5,4,0,0,0,1,1,3,3);
const papich = new wrestlerData('Vision', 'Papich', '-', 5,3,0,0,2,0,1,8,6);
const valakas = new wrestlerData('Vision', 'Glad Valakas', '-', 1,3,0,0,1,1,0,2,2);
...
const mobster = new wrestlerData('Vision', 'Mob5ter', '-', 2,3,0,0,0,2,0,0,0);
const nastjaDD = new wrestlerData('Vision', 'Nastjadd', '-', 0,2,0,0,0,0,1,2,0);
const ozon = new wrestlerData('Vision', 'Ozon', '-', 6,4,0,0,0,1,1,2,2);
const pahom = new wrestlerData('Vision', 'Pahom', '-', 2,0,0,0,0,0,0,1,1);
const pierreDunn = new wrestlerData('Vision', 'Pierre Dunn', 'Bosco Brothers', 4,3,1,0,0,2,0,2,1);
const pokimane = new wrestlerData('Vision', 'Pokimane', '-', 2,0,0,0,0,0,0,0,0);
const gosling = new wrestlerData('Vision', 'Ryan Gosling', '-', 3,5,0,0,0,0,2,0,0);
const grafoyni = new wrestlerData('Vision', 'Sgt. Grafoyni', 'Wycc Band', 0,1,0,2,0,0,0,0,0);
const snailkick = new wrestlerData('Vision', 'Snailkick', 'Success', 1,2,3,1,0,0,0,0,0);
const sunboy = new wrestlerData('Vision', 'Sunboy', '-', 0,6,0,0,0,0,0,1,1);
const toddHoward = new wrestlerData('Vision', 'Todd Howard', '-', 2,0,0,0,0,0,0,1,1);
const julik = new wrestlerData('Vision', 'Julik', 'Hangover', 0,0,3,0,0,0,0,0,0);
const vanomas = new wrestlerData('Vision', 'Vanomas', 'Перцы в Берцах', 0,0,2,2,0,0,0,1,0);
const zuev = new wrestlerData('Vision', 'Viktor Zuev', 'Gamer Industry', 1,1,2,3,0,0,0,0,0);
const vjlink = new wrestlerData('Vision', 'VJLink', '-', 1,2,0,0,0,0,1,1,1);
